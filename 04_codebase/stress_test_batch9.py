"""
Batch 9: Calendar Event Drift Strategies — Step 1 Stress Test
Tests macro event pre-announcement drift across equities, bonds, metals, energy.

All strategies use ONE_SHOT_IS_OOS (IS learns long/short direction).
Events: NFP (12/yr), CPI (12/yr), EIA Inventory (52/yr), ECB (8/yr), Fed Minutes (8/yr).

Rationale: FOMC drift on ES is a proven survivor. Testing adjacent event types
that also create predictable institutional repositioning windows.
"""
import sys
import os
import json
...
BATCH9_KEYS = [
    # Non-Farm Payrolls (12/yr — major jobs report, moves equities + bonds + Gold)
    "nfp_drift_es",
    "nfp_drift_zn",
    "nfp_drift_gc",
    "nfp_drift_cl",
    # CPI Inflation (12/yr — dominant macro driver 2021-2024)
    "cpi_drift_es",
    "cpi_drift_zn",
    "cpi_drift_gc",
    # EIA Crude/Gas Inventory (52/yr — weekly, high trade count)
    "eia_inventory_cl",
    "eia_inventory_ng",
    # ECB Rate Decision (8/yr — EUR-specific)
    "ecb_drift_fxe",
    # Fed Minutes (8/yr — delayed Fed communications)
    "fed_minutes_es",
    "fed_minutes_zn",
]
...
def main():
    ...
    print(f"{'='*60}")
    print(f"BATCH 9 — Calendar Event Drift Strategies")
    ...
    print(f"{'='*60}")
    print(f"DONE. {len(survivors)}/{len(BATCH9_KEYS)} PASS verdicts.")
    if survivors:
