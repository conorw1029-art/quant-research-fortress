"""
Batch 6: RTH ORB — Step 2 Extended Stress Suite
Runs rth_orb_gc (the sole Batch 6 Step 1 survivor) through six stress regimes.

TESTS:
  1. double_cost   — charge 2x realistic per-trade cost
  2. half_size     — halve position size
  3. slip_shock    — fat-tail random slippage (exponential dist)
  4. missed_20     — randomly drop 20% of signals
  5. stop_200      — hard $200 per-trade stop loss
  6. topstep       — Topstep Funded rules ($1500/day limit, $2000 trailing DD)

Results written to 05_backtests/batch6_step2.jsonl.
"""
...
HARDENED = [
    ...
    ("rth_orb_gc",   5.391,     1.924,     1023),
]

OUT = THIS_DIR.parent / "05_backtests" / "batch6_step2.jsonl"
...
def print_report(all_results: List[dict]):
    ...
    print("\n" + "=" * W)
    print("  BATCH 6 STEP 2: Extended Stress Suite — RTH ORB GC Survivor")
    ...
    print("\n" + "=" * W)
    print("  BATCH 6 STEP 2 OVERALL SUMMARY")
    print("=" * W)
...
def main():
    ...
    logger.info("="*65)
    logger.info("  BATCH 6 STEP 2: EXTENDED SURVIVOR STRESS SUITE")
