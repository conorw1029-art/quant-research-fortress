"""
Batch 10: Remaining Market Exhaustion — Intraday Strategies
Tests all 5 proven intraday strategy families on the last untested markets:
  6C (Canadian Dollar), 6A (Australian Dollar), YM/MYM (Dow Jones).

Hypothesis: Gold (MGC) uniqueness should hold — all 15 expected to fail.
Commodity-linked FX (CAD tracks oil, AUD tracks metals) is the best remaining hope.

15 tests total. Uses Walk-Forward Optimization.
"""
import sys
import os
import json
...
BATCH10_KEYS = [
    # Bollinger+RSI mean-reversion on commodity currencies and Dow
    "bollinger_rsi_fxc",
    "bollinger_rsi_fxa",
    "bollinger_rsi_ym",
    # VWAP reclaim/reject on commodity currencies and Dow
    "vwap_reclaim_fxc",
    "vwap_reclaim_fxa",
    "vwap_reclaim_ym",
    # RTH Opening Range Breakout on commodity currencies and Dow
    "rth_orb_fxc",
    "rth_orb_fxa",
    "rth_orb_ym",
    # Z-score volatility-adjusted momentum on commodity currencies and Dow
    "vol_adj_momentum_fxc",
    "vol_adj_momentum_fxa",
    "vol_adj_momentum_ym",
    # Intraday Donchian channel breakout on commodity currencies and Dow
    "donchian_intraday_fxc",
    "donchian_intraday_fxa",
    "donchian_intraday_ym",
]

COST_SCENARIO = "realistic"
RESULTS_PATH = os.path.join(os.path.dirname(__file__), "..", "05_backtests", "batch10_results.jsonl")
...
def main():
    ...
    print(f"{'='*60}")
    print(f"BATCH 10 — Remaining Market Exhaustion (Intraday)")
    print(f"Markets: 6C (CAD), 6A (AUD), YM/MYM (Dow)")
    ...
    print(f"{'='*60}")
    print(f"DONE. {len(survivors)}/{len(BATCH10_KEYS)} PASS verdicts.")
    if survivors:
